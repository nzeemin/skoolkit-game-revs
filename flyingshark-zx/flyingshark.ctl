...
N $9A00 256-byte lookup where entry i is i with its bit order reversed (bit 0<->7, 1<->6, ...). Used to horizontally mirror sprite bytes: $7D98 builds each sprite's mirrored half by bit-reversing every other byte through this table, and the masked tile blitters ($B1BE etc.) XOR the bit-reversed pixel byte to draw mirrored sprites.
...
W $A076,2,2 $3B - dispatch sub-command (C bits 3-6) via $A082
W $A078,2,2 $3C - set field IY+$19 (from C or next script byte)
W $A07A,2,2 $3D - set absolute heading, apply velocity
W $A07C,2,2 $3E - turn heading relative, apply velocity
W $A07E,2,2 $3F - set/clear mirror flag (IY+$04 bit 6)
W $A080,2,2 $40 - set N object fields from script (offset,value pairs)
...
N $A13A Reads the pending-events bitmask ($9C27) ANDed with an enable/priority mask ($9C28); finds the lowest-numbered surviving bit (highest priority), clears it, and dispatches its handler via the $A042 sub-range of the $A000 jump table. General deferred-event dispatcher (bits set by $66B5 bit 1 HUD-redraw, $A1EA bit 0, $A8CF/$BF57/$C33B/$C373 bit 2, $C8C9 bit 3) - not sound-specific.
...
N $A15F Restricts the event-enable mask $9C28 to bit 7 only, then redraws the active player's HUD panel ($6771). Dispatched by $A13A when the corresponding $9C27 bit is pending.
c $A16D Deferred event $28: clear event bit 0, load pending sound effect
N $A16D Clears bit 0 of the event-enable mask $9C28, then loads the pending sound effect via $A1F2. Dispatched by $A13A.
c $A178 Deferred event $27: mask event bits, redraw score and check bonus life
N $A178 Restricts the event-enable mask $9C28 to bit 7 only, then reprints the active player's score ($66DE) and awards a bonus life if the threshold was crossed ($A189). Dispatched by $A13A.
...
N $A6BB Called from $A8C0's attract loop. Draws the title screen (message $01, "FIREBIRD...FLYING SHARK...GRAFTGOLD LTD...COPYRIGHT 1987") and starts music track 0; returns early with carry set if fire is pressed. The $A6C9 entry then shows the credits (message $02, design/graphics/music by-line), runs the $C6A6 wipe, and waits ~200 frames or until fire.
...
C $A93B,5 End marker $FF: stop the track ($AA32)
C $A943,5 Index pattern-pointer table $AACB (2 bytes/entry)
...
C $A95C,5 End-of-score marker $FF: fetch next event ($AA2B)
C $A961,5 Below $60: play a note ($A98A)
C $A966,3 Exactly $60: reset instrument ($A97A)
C $A96B,3 $61-$6F: set instrument/octave value ($A972)
C $A970,2 $70-$CF: set note duration ($A981)
C $A972,5 Store instrument/octave low nibble ($AA98)
C $A984,3 Store note duration ($AA92)
C $A990,6 Index note-pitch table $AA9B (3 bytes/entry)
C $A996,4 Store the entry's tone byte to $AA8D
...
C $A9C1,4 Reload the tone byte ($AA8D)
...
N $AFC1 Handler for index $14 of the $A000 jump table (see $B7F6). A single JP (IX) - an indirect dispatch that continues execution at whatever address IX currently holds.
...
N $B121 Sets up a masked blit with destination $79F0 (into the 2x2 tile-block tables, not the screen) and jumps into the shared body at $B1C6 inside $B1BE.
C $B121,4 Source tile data via SP (POP-fed)
C $B125,3 Destination = tile-block buffer $79F0
C $B128,3 Run the shared masked-blit body ($B1BE)
c $B12B Masked tile blit setup (index $03): configure, then run $B24A body
N $B12B Handler for index $03 of the $A000 jump table (see $B7F6). Sets up a masked tile blit (source via SP, dest high = width+$78, self-modified row-start operand = $F0) and jumps into the shared blit body at $B25E inside $B24A.
C $B130,4 Source tile data via SP (POP-fed)
C $B134,3 Destination high byte = width + $78
C $B139,3 Self-modify $B24A's row-start operand ($F0)
C $B13C,3 Run the shared masked-blit body ($B25E)
c $B13F Masked tile blit setup (index $05): to tile-block table, run $B2DA body
N $B13F Sets up a masked blit with destination page $79 (into the 2x2 tile-block tables, not the screen) and jumps into the shared body at $B2E7 inside $B2DA.
C $B13F,4 Source tile data via SP (POP-fed)
C $B143,2 Destination page = $79 (tile-block table)
C $B147,3 Run the shared masked-blit body ($B2DA)
c $B14A Masked tile blit setup (index $07): run $B36A body
N $B14A Sets up a masked blit with destination high byte = width+$78 (screen) and jumps into the shared body at $B37F inside $B36A.
C $B14A,3 B = column/pass count ($9C1A)
C $B14F,4 Source tile data via SP (POP-fed)
C $B153,3 Destination high byte = width + $78
C $B158,3 Run the shared masked-blit body ($B36A)
...
N $B55E Handler for index $11 of the $A000 jump table (see $B7F6). Copies vertical 8-byte-wide strips of screen memory using an SP/EX (SP),HL trick to alternate source and destination pointers: each row LDIs 8 bytes then advances by $0024. Column source at $9C0F is range-checked against $C8 to pick which of three copy loops runs ($B577 plain, $B599 with INC H, $B5CE with DEC H) so the copy steps correctly across the ZX screen's non-linear thirds. Continues into the formation script ($AFB8).
...
N $B61A Handler for index $12 of the $A000 jump table (see $B7F6). Like $B55E, copies vertical 8-byte-wide screen strips via the SP/EX trick with three screen-third variants, but tests each strip's high byte (AND $7E) and skips the copy when zero - a transparent-strip blit. Continues into the formation script ($AFB8).
...
N $BA0A SP=IY stack trick: adds the velocity fields (set by $B8A2, IY+7/+8 and IY+9/+10) onto the position fields (IY+11/+12 and IY+13/+14). The generic "move by velocity" step; also index $43 of the $A000 jump table.
...
N $BA5F Called from $C981 (column spawn markers) and $C9AF (enemy timeline). Finds the first free bitmap slot at $9B4F, works out how many objects to spawn from the remaining wave counter ($9D18) against a per-group budget table at $5CF4, then pops that many objects off the free list ($9D1C) via an SP-trick and initializes each from a spawn-template table (index*9 + $55), applying a difficulty offset ($9C41). Returns carry set on success.
...
C $BAEF,6 Index template table (index*9 + $55)
C $BB07,8 Add difficulty offset ($9C41), clamp to $FF
...
N $BC7D Entered from $BBE5's per-object loop (and the $BC79 Immunity entry). ORs the object's mask bit into 8 consecutive rows of each of the $7Exx/$7Fxx attribute buffers - the fixed-height counterpart to $BCE9's variable-height object masks.
...
C $C5D1,1 return
c $C5D2 Convert object to explosion (set per-frame handler $6BF3)
@ $C5D2 label=object_to_explosion
R $C5D2 IY object struct base
N $C5D2 Called from $B79F on the $9C3F path. Increments $9C3F, positions the object via $B8DB, then points its per-frame handler (IY+$02/$03) at $6BF3 and inits state fields - the sibling of $C5AF (which uses handler $6BCA). The $C5EE entry (from $C648) additionally awards score (+8) and sets up a follow-up object based on the current player's state.
...
N $C772 Called every frame from the main loop. Uses an unrolled POP/self-modified-store SP-trick to move the scroll-window row prepared by $C84C into the on-screen attribute area, with $C825 selecting the per-frame store-opcode variant and $C804 advancing between row groups.
...
N $C8B8 Called every frame from $A0C4 and $A4E7. Advances a mod-8 frame counter at $9C00; only on the 8th frame does it request deferred event bit 3 ($9C27), toggle the ping-pong column-buffer selector ($9BAE), copy the active 18-byte column row into staging ($9BD4), call $C941 to decode the next column, and convert each row cell into an attribute-code pair (>=$60 clamped to $62) written to the on-screen scroll target ($9BF9).
...
N $C981 Called from $C941 when the "column ready" flag ($9BA2 bit 5) is clear. Scans 9 entries of the newly-decoded column buffer ($9BA9, stride $10) for spawn markers $05/$55; for each match, calls $BA5F to spawn a group (D = entry offset, used there as the enemy's row/type). $BA5F returns carry set on success: on a successful spawn the marker byte is left unchanged, but if the pool was full (carry clear) the cell is overwritten with $33 to mark it consumed/blocked.
...
N $CA09 Each frame is 24px wide x 16px tall = 3 vertical 8px strips of 32 bytes (16 rows of a [pixel, mask] pair). The mask byte is stored bit-reversed and must be flipped through the $9A00 table to align with the pixel byte. Successive frames are pre-shifted horizontal copies (each +1px) built by $7E4A for smooth sub-pixel movement; $7DF3 interleaves the pixel/mask pairs and $7D98 generates mirrored halves via $9A00.
