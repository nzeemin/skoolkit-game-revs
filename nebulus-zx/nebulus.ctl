...
C $81E7,3 Reset tower scroll and column position
C $81ED,3 Deactivate all enemies
...
c $8F31 DRAW_TOWER_CELL
N $8F31 Decode one tower cell descriptor byte at (HL): the sign bit and low 2/4 bits select which feature renderer (door / block / gap, pointers $8F8B etc.) to run.
N $8F31 Clobbers: A,HL,...
b $8F75
@ $8FB7 label=clear_actor_states
c $8FB7 CLEAR_ACTOR_STATES
...
c $95BD CHECK_COLLISIONS
N $95BD Test actor IX against the four object_table entries for an overlap (runs with interrupts off).
...
c $A700 PROC_TABLE
N $A700 Jump/trampoline dispatch table, $A700..$A79E (53 entries, each 'JP nnnn', 3 bytes).
...
c $AA5B SET_LEVEL_TIMER
N $AA5B Load play_timer from the word table at $A8B7 indexed by #R$6039*2 (per-level start time), then fall into draw_timer to show it.
...
C $B633,1 Low byte from preceding word entry@ $B636 label=select_player_sprite
c $B636 SELECT_PLAYER_SPRITE
...
c $B7C2 BLIT_SPRITE_FAST
N $B7C2 Fast sprite blit using the LD SP,screen / PUSH stack-write trick (saves the real SP to $6000, DI).
N $B7C2 Height B is 8 or 4 (chosen by the $4E threshold) C selects a sub-variant.
@ $B869 label=actor_screen_y
c $B869 ACTOR_SCREEN_Y
